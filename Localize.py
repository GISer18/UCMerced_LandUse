...
new_class_rgb_value = np.vstack((class_rgb_value[2],class_rgb_value[10],class_rgb_value[-2],[255,255,255]))
#%%
multilabel = True
model_name = 'downclass1-finetune.84-0.30'
if multilabel:
    data_name = 'full_traintest3'
else:
    data_name = 'partial_traintest3'
...
if multilabel:
    ...
    plt.figure()
    plot_confusion_matrix(cnf_matrix, classes=new_class_names,
...
if multilabel:
    ...
    print(y_pred[i])
    print(preds_class[0])
    print('Predict =',[new_class_names[idx] for idx in preds_class[0]])
    plt.imshow(x_test[i])      
else:
    print(y_true[i])
    print(y_pred[i])
...
if multilabel:
    ...
    for idx,class_idx in enumerate(preds_class[0]):
        plt.figure(idx+2)
        plt.xlabel(new_class_names[class_idx])
...
#        threshold = CAM.max()*0.1 #threshold CAM
#        CAM[CAM<threshold] = 0 
#        plt.figure(99-idx)
#        plt.xlabel(new_class_names[class_idx])
#        plt.imshow(CAM,cmap='jet')
...
#        mask[location[0],location[1],:] = new_class_rgb_value[class_idx]      
#        plt.imshow(mask)
    plt.figure(idx+4)
